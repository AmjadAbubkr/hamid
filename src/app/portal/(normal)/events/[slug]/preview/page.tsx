...
export default async function EventPreviewPage({ params }: { params: Promise<{ slug: string }> }) {
  unstable_noStore();
  ...
  const { data, error } = await supabase
    .from("upcoming_event")
    .select("slug, status, title_ar, title_fr, title_en, body_ar, body_fr, body_en, event_date, venue_ar, venue_fr, venue_en, institution_ar, institution_fr, institution_en, role, role_other_ar, role_other_fr, role_other_en, registration_url, image_path")
    .eq("slug", slug)
  ...
  return (
    <PortalFrame title="Upcoming Event preview">
      <div className="flex flex-wrap gap-4 text-sm font-semibold">
        <Link href={`/portal/events/${data.slug}`} className="text-zinc-950 underline underline-offset-4"><PortalText>Back to edit</PortalText></Link>
        <p className="text-zinc-700">{data.status === "draft" ? "Draft preview - not visible on the Profile." : "Public event preview."}</p>
      </div>
      <section className="flex flex-col gap-6 rounded-lg border border-zinc-300 bg-white p-5">
        ...
        <p className="text-zinc-700">{data.event_date || "Event date not set"}</p>
        ...
        <dl className="grid gap-3 sm:grid-cols-2">
          <Detail term="Role" value={data.role === "Other" ? data.role_other_fr || data.role_other_ar : data.role ? getParticipationRoleEditorLabel(data.role) : null} />
          <Detail term="Registration URL" value={data.registration_url} isUrl />
        </dl>
      </section>
    </PortalFrame>
  );
...
function Detail({ term, value, isUrl = false }: { term: string; value: string | null; isUrl?: boolean }) {
  const safeUrl = isUrl ? safeHttpUrl(value) : null;

  return (
    <div className="flex flex-col gap-1">
      <dt className="text-sm font-medium text-zinc-600">{term}</dt>
      <dd className="text-zinc-950">
        {value ? (safeUrl ? <a href={safeUrl} className="underline underline-offset-4" rel="noreferrer">{value}</a> : value) : "Not set"}
      </dd>
    </div>
  );
}
