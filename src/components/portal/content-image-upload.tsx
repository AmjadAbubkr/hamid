...
export function ContentImageUpload({ file, onFileChange, disabled }: { file: File | null; onFileChange: (file: File | null) => void; disabled?: boolean }) {
  return <section className="flex flex-col gap-3 rounded-lg border border-zinc-300 bg-white p-4">
    <div><h2 className="font-semibold text-zinc-950">Optional image</h2><p className="text-sm text-zinc-600">JPEG, PNG, or WebP up to 8 MB. It uploads when you save.</p></div>
    <input aria-label="Optional image" type="file" accept="image/jpeg,image/png,image/webp" disabled={disabled} onChange={(event: ChangeEvent<HTMLInputElement>) => onFileChange(event.target.files?.[0] ?? null)} />
